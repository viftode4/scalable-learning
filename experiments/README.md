...
`table1_local_medium.yaml` is the stronger local rung: same RoBERTa-base QNLI setup as the pilot, but 10 rounds and 5 local batches. Run one mode first with `make table1-medium MODE=rolora`; run all modes only if runtime is acceptable.
