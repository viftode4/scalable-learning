...
COMMON_TAGS="local,proxy,qnli,roberta-base,c50,r4,rolora,svd_compensated,monitor,bba,phase-controller,tmux-retry"
run_arm() {
  ...
  export WANDB_RUN_GROUP=qnli_c50_r4_svd_bba_tmux_retry
...
echo "===== $(date '+%Y-%m-%d %H:%M:%S %Z') SVD BBA TMUX RETRY DONE failures=$failures ====="
exit "$failures"
