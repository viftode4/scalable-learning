...
- DelftBlue full access is granted under portal share `Education-EEMCS-MSc-DSAIT`; the sbatch files use Slurm account `education-eemcs-msc-dsait`. Slurm accounting may lag the 2026-06-01 grant by up to one working day.
...
Full paper Table 1 is RoBERTa-Large across MNLI/QQP/QNLI, 3/20/50 clients, three methods, and multiple seeds. That is a cluster job (hundreds of GPU-hours). Local runs are pipeline and mechanism evidence, not paper-comparable Table 1 numbers. The previous C2 cluster jobs reached chance because they used the under-tuned shipped SGD recipe and too few rounds for 20 clients; the next DelftBlue evidence gate is the corrected C2 RoLoRA seed-0 relaunch on `main` using AdamW, 75 rounds, and the confirmed account share.

## Status
**Week 3 — cluster pipeline corrected after first failed evidence.** Main env is pinned, MNIST sanity checks run locally, the authors' supplement is installed in an isolated Python 3.9 env, local Table-1-shaped pilots are runnable, and **RoBERTa-Large feasibility is verified on Apple MPS** (the supplement now falls back from CUDA → MPS → CPU). Daniel's `main` work added the reproducibility audit, corrected trainer behavior, AdamW/round-count diagnosis, overnight runners, and failed-cluster evidence. The current next step is to sync `main`, confirm DelftBlue's new account share is active in Slurm, relaunch corrected C2 RoLoRA seed 0, and only then fan out to the other C2 modes/seeds. See [`docs/progress.md`](docs/progress.md) for full status, [`docs/experiment-matrix.md`](docs/experiment-matrix.md) for the matrix, and [`docs/kickoff.md`](docs/kickoff.md) for remaining team/process items.
