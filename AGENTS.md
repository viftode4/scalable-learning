...
- **Cluster access is available** on DelftBlue (and DAIC as backup). Full access was granted on 2026-06-01 under portal share `Education-EEMCS-MSc-DSAIT`; the Slurm templates use the lower-case account directive `education-eemcs-msc-dsait`. Slurm's accounting database may lag the grant by up to one working day. Open work is the first paper-scale submission. See `docs/setup/delftblue.md` and `slurm/README.md` for the current state.
