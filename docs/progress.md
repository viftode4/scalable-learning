...
| 2026-06-13 | Added a teammate-facing improvement handoff and direct toy improvement bank, then re-checked actual SVD proxy logs. | Prevents the team from mistaking the new toy SVD signal for the actual-model priority: SVD was already run on QNLI proxy and is only small/neutral; current strongest actual-model candidates remain orthogonal-A+BBA and adaptive refresh+orthogonal-A. | `docs/improvement-handoff-2026-06-13.md`, `scripts/run_toy_improvement_bank.py`, `evidence/toy_improvement_bank_label_shard_combined.md`, `results/overnight_proxy_svd_compensated_c50_r4_lr1e-2_seed{0,1,2}.log`, `experiments/ledger/README.md` |
